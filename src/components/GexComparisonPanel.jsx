            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-700">
                  <th className="text-left text-gray-400 font-medium px-2 py-1.5">Level</th>
                  <th className="text-right text-gray-400 font-medium px-2 py-1.5">CBOE</th>
                  <th className="text-right text-gray-400 font-medium px-2 py-1.5">Tradier</th>
                </tr>
              </thead>
              <tbody>
                {/* Main levels */}
                {mainLevels.map((row, i) => (
                  <tr key={row.key} className={i % 2 === 0 ? 'bg-gray-800' : 'bg-gray-750'}>
                    <td className={`px-2 py-1.5 ${row.color}`}>{row.label}</td>
                    <td className={`text-right font-mono px-2 py-1.5 ${row.color}`}>
                ...
                <tr className="bg-gray-700">
                  <td colSpan={3} className="px-2 py-1 text-orange-400 font-medium">Resistances</td>
                </tr>
                {/* Resistance values - high to low (R5 to R1) */}
                {[5, 4, 3, 2, 1].map((n, i) => (
                  <tr key={`r${n}`} className={i % 2 === 0 ? 'bg-gray-800' : 'bg-gray-750'}>
                    <td className="px-2 py-1 text-orange-400 text-gray-500"></td>
                    <td className="text-right font-mono px-2 py-1 text-orange-400">
                ...
                <tr className="bg-gray-700">
                  <td colSpan={3} className="px-2 py-1 text-cyan-400 font-medium">Supports</td>
                </tr>
                {/* Support values - high to low (S1 to S5) */}
                {[1, 2, 3, 4, 5].map((n, i) => (
                  <tr key={`s${n}`} className={i % 2 === 0 ? 'bg-gray-800' : 'bg-gray-750'}>
                    <td className="px-2 py-1 text-cyan-400"></td>
                    <td className="text-right font-mono px-2 py-1 text-cyan-400">
